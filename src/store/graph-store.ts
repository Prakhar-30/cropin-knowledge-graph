...
export async function pullDocument(client: SupabaseClient, tenantId: string): Promise<GraphDocument> {
  ...
  const doc = {
    ...
    concepts: concepts
      .map((c) => ({
        id: String(c.id), key: String(c.key), label: String(c.label), layer: String(c.layer),
        definition: String(c.definition ?? ''), decide: String(c.decide ?? ''), source: String(c.source ?? ''),
        ...(c.label_scope ? { label_scope: String(c.label_scope) } : {}),
    ...
    links: links
      ...
      .sort((a, b) => a.rel.localeCompare(b.rel) || a.from.localeCompare(b.from) || a.to.localeCompare(b.to)),
  };
